...
do
	...
	read -p "Please enter an interger: " user_guess
	if [[ $user_guess =~ ^[+-]?[0-9] ]];then
		echo -e "\nYou entered $user_guess which is an integer\n"
		break
	else
		echo -e "\nThat is not an interger. Press Enter to try again.\n"
		read
		clear
	fi
...
do
	echo -e "Task 6\n"
	read -p "Please enter a last name (Mysse): " user_name
	if [[ $user_name == "Mysse" ]];then
		break
	else
		clear
		continue
