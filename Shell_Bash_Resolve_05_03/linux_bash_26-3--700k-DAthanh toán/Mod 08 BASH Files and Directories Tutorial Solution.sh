...
#we don't need to check for employee.csv
#we will write the header with >, which is destructive
...
echo "Name,Address,City" > ~/mod08t/employee.csv #this is destructive on purpose

#write the employee data out to employee.csv in the mod08t directory
array_len=${#employee_array[*]}
for (( i=0; i<$array_len; i=$(($i + 3)) ))
do
	#be sure to write out like this so no line length errors
	line_out="${employee_array[$i]},\
${employee_array[$(($i+1))]},\
${employee_array[$(($i+2))]}"
	echo -e $line_out >> ~/mod08t/employee.csv
done
...
#sort and unique the list of cities
city_array=($(
	for i in ${city_array[*]};
	do
	echo $i
...
#add last names to city strings in array
...
#print out the cities and last name in a clean fashion
clear
for i in ${city_array[*]}
do
...
#check for emp_data.tgz, delete if there
filecheck=~/mod08t/emp_data.tgz
if [[ -f $filecheck ]];then
	rm ~/mod08t/emp_data.tgz
fi

#pack up employee_data.csv into the mod08t directory
cd ~/mod08t #go there to only pack files, not directories
